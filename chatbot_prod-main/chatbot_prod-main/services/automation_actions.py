...
async def execute_whatsapp_message(context, node_id: str, config: Dict, trigger_data: Dict):
    """
    Execute a WhatsApp message node
    ...
    if platform != "whatsapp":
        context.log(
            ...
            "whatsapp_message", 
            "error", 
            "Invalid platform for WhatsApp message", 
            success=False
        )
        return
    ...
    context.log(node_id, "whatsapp_message", "executing", f"Sending WhatsApp message: {text[:50]}...")
    ...
    try:
        ...
        if not conversation_id or not whatsapp_id:
            context.log(
                ...
                "whatsapp_message", 
                "error", 
                "Missing conversation_id or whatsapp_id", 
                success=False
            )
            return
        ...
        result = await send_whatsapp_message(
            ...
            mode="automation"  # Special mode for automated messages
        )
