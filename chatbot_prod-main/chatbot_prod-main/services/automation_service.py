...
async def publish_automation_flow(org_id: str, flow_id: str) -> Optional[Dict]:
    ...
    Publish an automation flow:
      1. Validate the flow has at least one trigger
      2. Register all triggers in automation_triggers collection
      3. For whatsapp_followup triggers: schedule the follow-up job(s)
      4. Mark the flow as published
    """
    collection = _automation_flows_collection(org_id)
    ...
    if not triggers:
        raise ValueError("Flow must have at least one trigger before publishing")

    # ── Register triggers (for webhook-driven automations) ─────────────────
    await _register_flow_triggers(org_id, flow_id, flow_data)

    # ── Schedule WhatsApp follow-up jobs (publish-time scheduling) ─────────
    ...
    for trigger in triggers:
        ...
        if not conversation_id:
            logger.warning(
                f"⚠️ whatsapp_followup trigger in flow {flow_id} "
                "has no conversation_id — skipping"
            )
            continue

        try:
            job_id = await schedule_whatsapp_followup(
                org_id=org_id,
                flow_id=flow_id,
                conversation_id=conversation_id,
                flow_data=flow_data,   # Let scheduler derive delay + start node
            )
            logger.success(
                f"✅ Scheduled follow-up job {job_id} for "
                f"conv={conversation_id}"
            )
        except Exception as e:
            logger.error(
                f"❌ Failed to schedule follow-up for {conversation_id}: {e}"
            )
            # Don't block publish — the flow is still activated
            # Admin can see the failed job in automation_scheduled_jobs
    ...
    if updated_flow:
        logger.success(f"✅ Published flow {flow_id}")
...
async def unpublish_automation_flow(org_id: str, flow_id: str) -> Optional[Dict]:
    """
    Unpublish a flow:
      1. Deactivate all registered triggers
      2. Cancel all pending follow-up jobs
      3. Mark flow as draft
    """
    collection = _automation_flows_collection(org_id)
    ...
    # ── Deactivate webhook triggers ────────────────────────────────────────
    await _deactivate_flow_triggers(org_id, flow_id)

    # ── Cancel scheduled follow-up jobs ───────────────────────────────────
    try:
        from services.automation_scheduler import cancel_flow_followups
        cancelled = await cancel_flow_followups(org_id, flow_id, reason="flow_unpublished")
        logger.info(f"Cancelled {cancelled} follow-up job(s) for flow {flow_id}")
    except Exception as e:
        logger.warning(f"Could not cancel follow-up jobs: {e}")
    ...
    if updated_flow:
        logger.info(f"Unpublished flow {flow_id}")
        return serialize_mongo(updated_flow)
...
async def _register_flow_triggers(org_id: str, flow_id: str, flow_data: Dict):
    """
    Register all triggers from a flow into automation_triggers.
    whatsapp_followup triggers are NOT registered here because they are
    publish-time scheduled, not event-driven.
    """
    triggers_collection = _automation_triggers_collection()
    triggers = flow_data.get("triggers", [])
    ...
    for idx, trigger_config in enumerate(triggers):
        ...
        # whatsapp_followup is handled by the scheduler, not the trigger handler
